<template>
  <div class="container mt-4">
    <div class="header d-flex justify-content-between">
      <h1>Edit Admin</h1>
      <button @click="deleteAdmin" class="btn btn-danger align-self-center">
        x
      </button>
    </div>
...
export default {
  ...
  methods: {
    // to get admin based on id given by param from route
    async getAdmin() {
    ...
    async updateMember() {
      ...
      // it will insert "gambar_lama" request with gambar from admin data
      ...
      try {
        ...
        if (res.status == 200) {
          this.$toasted.show("Berhasil mengupdate admin", {
            type: "success",
            duration: 3000
          });
    ...
    async deleteAdmin() {
      try {
        var res = await axios.post("/admin/" + this.$route.params.id, {
          _method: "delete"
        });
        if (res.status == 200) {
